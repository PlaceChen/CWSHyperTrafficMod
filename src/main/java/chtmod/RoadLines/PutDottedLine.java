...
public class PutDottedLine extends Item {
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos,
		...
		if (stack.hasTagCompound()) {
			...
			if (worldIn.getBlockState(new BlockPos(x0, y, z0))
					.getBlock() instanceof chtmod.RoadLines.Curved.LineMarker) {
	...
	private IBlockState getStateByMeta(int meta) {
		...
		switch (meta) {
		case 0:
			return white.getDefaultState().withProperty(RoadLine.PROPERTYANGLE, EnumAngle.A);
		case 1:
			return yellow.getDefaultState().withProperty(RoadLine.PROPERTYANGLE, EnumAngle.A);
		case 2:
			return white.getDefaultState().withProperty(RoadLine.PROPERTYANGLE, EnumAngle.B);
		case 3:
			return yellow.getDefaultState().withProperty(RoadLine.PROPERTYANGLE, EnumAngle.B);
		}
	...
	private void putDottedStraightLine(World world, EnumFacing facing, BlockPos start, int length, IBlockState state,
			boolean slope) {
		int counter = 0;
		if (!(state.getBlock() instanceof RoadLine))
			return;
			...
			if (counter < 3)
				...
				case EAST:
					world.setBlockState(start.east(i).north(j),
							state.withProperty(RoadLine.PROPERTYFACING, EnumFacing.EAST));
					break;
				case NORTH:
					world.setBlockState(start.north(i).west(j),
							state.withProperty(RoadLine.PROPERTYFACING, EnumFacing.NORTH));
					break;
				case SOUTH:
					world.setBlockState(start.south(i).east(j),
							state.withProperty(RoadLine.PROPERTYFACING, EnumFacing.SOUTH));
					break;
				case WEST:
					world.setBlockState(start.west(i).south(j),
							state.withProperty(RoadLine.PROPERTYFACING, EnumFacing.WEST));
					break;
				default:
					break;
				}
